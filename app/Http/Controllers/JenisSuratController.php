...
class JenisSuratController extends Controller
{
    ...
    public function index()
    {
        ...
        $unit = DB::table('unit')
                ->select('*')
                ->get();
        return view('master.jenis_surat',compact('user','unit'));
    }
    function get_list(Request $request){
        $roles = DB::table('jenis_surat as a')
                ->join('unit as b','a.kd_unit','=','b.kd_unit')
                ->selectRaw('a.kd_unit,a.nama,b.nama_unit,a.id')
                ->get();
    ...
    function save(Request $request){
        ...
        $check = DB::table('jenis_surat')->select('*')->where('kd_jenis',$request->post('kd_jenis'))->count();
        if($check>0){
            $datas =array("msg"=>"<b style='color:red'>Kode Jenis Sudah Terdaftar </b>","code"=>0);
            echo json_encode($datas);exit;
        }
        $insert = DB::table('jenis_surat')->insert($data);
        if($insert){
